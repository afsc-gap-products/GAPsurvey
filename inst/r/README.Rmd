...
This repository uses automated GitHub Actions workflows to streamline package updates and documentation builds. Follow these steps to release a new version:

Step 1: Update the Version Number in DESCRIPTION
The only manual change you need to make is updating the version number in the DESCRIPTION file at the repository root. The version uses a date-based format (YYYY.MM.DD):
...
bash
git add DESCRIPTION
git commit -m "Update version to 2026.04.21"
...
✅ Runs R package checks on Windows (R release and R 3.6.0) to ensure code quality
...
When you update the version in DESCRIPTION and push to main, the following are automatically updated:

.tar.gz package file - Built with the new version number
inst/r/GAPsurvey-script.R - Regenerated from the vignette
README.md - Rendered from README.Rmd (this file)
GitHub Pages documentation - Rebuilt and deployed
GitHub Actions artifacts - .tar.gz available for download
Troubleshooting
Workflow failed? Check the GitHub Actions tab for detailed error logs
Version not updating? Ensure you edited the Version: line in DESCRIPTION (not Date:)
Files not committing back? Verify the workflow has permissions to commit (should be automatic)
